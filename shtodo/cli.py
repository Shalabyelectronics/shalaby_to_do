"""This module provides the SH To-Do CLI."""
# shtodo/cli.py
...
def init(
    ...
    if app_init_error:
        typer.secho(
                f"Creating config file faild with '{ERRORS[app_init_error]}'",fg=typer.colors.RED,
                )
        raise typer.Exit(1)
    db_init_error = database.init_database(Path(db_path))
    if db_init_error:
        typer.secho(
            f"Creating database faild with'{ERRORS[db_init_error]}'",
            fg=typer.colors.RED,
                )
        raise typer.Exit(1)
    else:
...
def get_todoer() -> shtodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Cofig file not found. Please, run "shtodo init"',
            fg=typer.colors.RED,
                )
        raise typer.Exit(1)
    if db_path.exists():
        return shtodo.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "shtodo init"',
            fg=typer.colors.RED,
                )
        raise typer.Exit(1)
...
def add(
    ...
    if error:
        typer.secho(
                f'Adding to-do faild with "{ERRORS[error]}"',
                fg=typer.colors.RED
                )
        raise typer.Exit(1)
    else:
...
def set_done(todo_id: int = typer.Argument(...)) ->  None:
    ...
    if error:
        typer.secho(
            f'Completing to-do # "{todo_id}" faild with "{ERRORS[error]}"',
            fg=typer.colors.RED,
                )
        raise typer.Exit(1)
    else:
...
def remove(
    ...
    def _remove():
        ...
        if error:
            typer.secho(
                f'Removing to-do # {todo_id} failed with "{ERROR[error]}"',
                fg=typer.colors.RED,
                    )
            raise typer.Exit(1)
        else:
...
def remove_all(
    ...
    if force:
        ...
        if error:
            typer.secho(
                f'Removing to-dos faild with "{ERRORS[error]}"',
                fg=typer.colors.RED,
                    )
            raise typer.Exit(1)
        else:
